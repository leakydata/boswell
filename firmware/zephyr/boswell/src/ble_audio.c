...
void ble_audio_publish_info(void)
{
    ...
    /* The threshold the device is actually using, which is not necessarily
     * the one this host last asked for: it lives in cfg_store and survives a
    ...
    info_buf[43] = (uint8_t)((nd >> 24) & 0xFF);
